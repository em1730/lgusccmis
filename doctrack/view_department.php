...
if (isset($_GET['id'])) {
  ...
  $get_department_sql = "SELECT * FROM tbl_department WHERE objid = :id";
  $department_data = $con->prepare($get_department_sql);
  $department_data->execute([':id' => $objid]);
  while ($result = $department_data->fetch(PDO::FETCH_ASSOC)) {
    $objid = $result['objid'];
